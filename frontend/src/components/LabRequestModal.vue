<template>
  <ion-modal :is-open="isOpen" @didDismiss="closeModal" class="lab-request-modal">
    <div class="modal-wrapper">
      <div class="modal-header">
        <span class="modal-title">Laboratory Request</span>
      ...
      <div class="preview-area preview-a4">
        <iframe
          ...
          :src="pdfPreviewUrl"
          class="pdf-iframe"
          title="Laboratory request document"
        />
...
export default defineComponent({
  name: "LabRequestModal",
  components: { IonModal, IonIcon },
  props: {
    form: { type: Object, required: true },
    selectedTests: { type: Array as () => string[], default: () => [] },
    otherLabInput: { type: String, default: "" },
    ...
    generatedFilename: { type: String, default: "lab_request.docx" },
  ...
  setup(props, { emit }) {
    ...
    const buildPayload = () => {
      const selectedTests = Array.isArray(props.selectedTests) ? props.selectedTests : [];
      return {
        ...
        selectedTests,
        otherTests: (props.otherLabInput ?? "").toString().trim(),
        impression: (props.impression ?? "").toString().trim(),
        remarks: (props.remarks ?? "").toString().trim(),
      };
    };
    ...
    const fetchPdfPreview = async () => {
      ...
      try {
        const res = await axios.post(
          `${baseUrl}/api/doctor/lab-request/generate`,
          buildPayload(),
    ...
    watch(() => props.generatedDocxBlob, () => {
      if (props.isOpen) fetchPdfPreview();
    });
    ...
    const saveAsWord = () => {
      ...
      a.href = url;
      a.download = props.generatedFilename || "lab_request.docx";
      a.click();
...
<style scoped>
.lab-request-modal::part(content) {
